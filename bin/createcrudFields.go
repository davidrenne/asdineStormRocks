...
func main() {
	...
	app.Initialize("src/github.com/davidrenne/asdineStormRocks", "webConfig.json")
	settings.Initialize()
	...
	serverSettings.Initialize(os.Getenv("GOPATH")+"/src/github.com/davidrenne/asdineStormRocks", "webConfig.json")
...
`, inputFields[i].Name, capCamel, inputFields[i].Name, inputFields[i].Name)
	...
	if hasNameField {
		...
		`)
		utils.ReplaceTokenInFile(br, "-QUERIES-", `"github.com/davidrenne/asdineStormRocks/queries"`)
	} else {
		utils.ReplaceTokenInFile(br, "-COPY_NAME-", "")
